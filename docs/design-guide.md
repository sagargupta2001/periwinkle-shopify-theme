...
| **Journal article featured image** | 3:2 | 2400 × 1600 | Hard crop: 3:2 from 750px, **4:5 on phones**, so keep the subject in the central square. Shown up to 1220px wide, so upload at least 2400px wide. |
| **Journal gallery (`custom.gallery`)** | 4:5 | 1600 × 2000 | Hard 4:5 crop, 2 columns on phones and 3 on desktop, staggered. Alt text doubles as the visible caption. |
...
| **Footer paper background** | Original artwork ratio | — | Stretched to fit (`background-size: 100% 100%`). It's a CDN file referenced from the footer block in `base.css`. See customizations §9. |
